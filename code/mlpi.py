...
Current workflow:
  ...
  3. Load the externally generated ODME matrix and map its district/HQ zones
     onto the road graph.
...
DEFAULT_INPUT = INPUT_DIR / "Input.xlsx"
DEFAULT_GPKG_DIR = INPUT_DIR
DEFAULT_MASTER_GPKG = INPUT_DIR / "Nepal.gpkg"
DEFAULT_OUT_DIR = OUTPUT_DIR / "MLPI"
DEFAULT_ODME_MATRIX = OUTPUT_DIR / "OD" / "od_matrix.csv"
DEFAULT_AIRPORT_RTFD = INPUT_DIR / "Airport Table.rtfd"
...
class MLPIConfig:
    ...
    od_matrix_path: Path = DEFAULT_ODME_MATRIX
    od_matrix_source: str = "Symmetric district-level ODME matrix (vehicles/day)"
    min_od_pair_demand: float = 1.0
    seed: int = 42
    gen_maps: bool = True
    gen_figs: bool = True
    top_n: int = 20
    ...
    road_aadt_units: str = "vehicles/day"
    ...
    od_matrix_raw_positive_pcu_per_day: float = 0.0
    od_matrix_total_pcu_per_day: float = 0.0
    od_pairs_loaded: int = 0
    od_pairs_below_threshold_skipped: int = 0
    od_pcu_below_threshold_skipped: float = 0.0
    od_pairs_same_node_skipped: int = 0
    od_same_node_pcu_skipped: float = 0.0
    od_baseline_connected_pairs: int = 0
    od_baseline_connected_pcu_per_day: float = 0.0
    od_baseline_disconnected_pairs: int = 0
    od_baseline_disconnected_pcu_per_day: float = 0.0
...
def load_config(
    ...
    if rc_name:
        ...
        cfg.min_od_pair_demand = max(
            0.0,
            float(
                _param_get(
                    rc,
                    "Minimum ODME PCU demand for MLPI",
                    "MIN_OD_PAIR_DEMAND",
                    "Minimum OD pair demand",
                    default=cfg.min_od_pair_demand,
                )
            ),
        )
    if gpkg_dir is not None:
    ...
    if np_name:
        ...
        cfg.road_aadt_units = str(_param_get(np_, "Road AADT units", "ROAD_AADT_UNITS", default=cfg.road_aadt_units)).strip() or cfg.road_aadt_units
        cfg.mean_speed_factor = float(_param_get(np_, "Mean speed factor", "MEAN_SPEED_FACTOR", default=cfg.mean_speed_factor))
        cfg.speed_road = float(_param_get(np_, "Road free-flow speed km/h", "SPEED_ROAD", default=cfg.speed_road))
    ...
    # weight, model-parameter, socioeconomic, OD and formula sheets.

    wm_name = _sheet_name(available, "03_Weights")
    if wm_name:
    ...
    sw_name = _sheet_name(available, "03_Weights")
    if sw_name:
        ...
        cfg.w_s3 = 0.0
        cfg.w_e1 = float(_param_get(sw, "Economic E1 — OD PCU-hour/freight loss", "Economic E1 - OD PCU-hour/freight loss", "Economic E1 - AADT PCU-hour/freight loss", "Economic E1 — AADT flow loss", default=cfg.w_e1))
    ...
    cf_name = _sheet_name(available, "04_Model_Params")
    if cf_name:
        ...
        cfg.airport_access_km = float(_param_get(cf, "Airport ground-access radius", "AIRPORT_ACCESS_KM", default=cfg.airport_access_km))
    ...
    tr_name = _sheet_name(available, "06_Border_Trade")
    if tr_name:
    ...
    pop_name = _sheet_name(available, "07_Population_GDP")
    ...
    if pop_name:
        ...
        has_unified_zone_schema = {"district_name", "population", "gdp per capita npr", "longitude", "latitude"}.issubset(present_norm)
        if not has_unified_zone_schema and not {"district", "population", "gdp per capita npr", "longitude", "latitude"}.issubset(present_norm):
    ...
    cfg.named_place_source = f"Input.xlsx sheet {pop_name}" if pop_name and workbook_places else "Nepal.gpkg settlement layer / district zone anchors"
    cfg.place_lookup = workbook_places
    seen_places = set()
    dedup_places = []
    ...
    formula_name = _sheet_name(available, "08_Formula_Index")
    if formula_name:
    ...
    if not cfg.od_matrix_path.exists():
        raise FileNotFoundError(
            f"ODME matrix not found: {cfg.od_matrix_path}. "
            "Provide the calibrated ODME CSV before running MLPI."
        )
...
MASTER_GPKG_LAYERS = {
    ...
    "country": ("national_boundary", "country"),
...
def _harmonize_master_roads(gdf: gpd.GeoDataFrame, cfg: MLPIConfig) -> gpd.GeoDataFrame:
    ...
    link_len_col = first_col("link_len", "link_length", "length_km", "length")
    aadt_col = first_col(cfg.road_aadt_field)
    ...
    if not link_len_col:
        raise ValueError(
            "Nepal.gpkg layer NH is missing the required link_len field "
            "needed to use official road lengths."
        )
    out["source_link_len_km"] = out[link_len_col].map(_positive_number)
    out["source_link_len_field"] = link_len_col
    missing_link_len = out["source_link_len_km"].isna()
    if missing_link_len.any():
        raise ValueError(
            f"Nepal.gpkg layer NH field {link_len_col!r} has "
            f"{int(missing_link_len.sum())} missing or non-positive road lengths."
        )
    if aadt_col:
        out["aadt_source_value"] = pd.to_numeric(out[aadt_col], errors="coerce")
        out["aadt_source_field"] = aadt_col
        out["aadt_source_units"] = cfg.road_aadt_units
    else:
        raise ValueError(
...
def _source_length_from_attrs(attrs: Dict[str, Any]) -> Tuple[Optional[float], str]:
    length = _positive_number(attrs.get("source_link_len_km"))
    if length is not None:
        field = str(attrs.get("source_link_len_field", "link_len") or "link_len")
        return length, f"Nepal.gpkg NH {field}"
    length = _positive_number(attrs.get("link_len"))
    if length is not None:
        return length, "Nepal.gpkg NH link_len"
    return None, "missing_official_link_length"
...
def build_network(cfg: MLPIConfig, geo: Dict[str, gpd.GeoDataFrame]) -> Tuple[nx.Graph, Dict[Tuple[int, int], List[Tuple[float, float]]], List[Tuple[List[Tuple[float, float]], float, str, int]]]:
    ...
    for idx, pts, attrs in roads:
        geometry_km = _km(pts)
        source_km, length_source = _source_length_from_attrs(attrs)
        if source_km is None:
            raise ValueError(f"Road feature {idx} is missing a usable official link length.")
        km = source_km
        attrs = {**attrs, "_geometry_length_km": geometry_km, "_model_length_source": length_source}
    ...
    if not score_roads and roads:
        auto_min = max(
            1.0,
            np.percentile([
                _source_length_from_attrs(r[2])[0]
                for r in roads
                if _source_length_from_attrs(r[2])[0] is not None
            ], 90),
        )
    ...
    for orig_idx, pts, attrs, _km_value in score_roads:
        source_feature_km, _length_source = _source_length_from_attrs(attrs)
        if source_feature_km is None:
            raise ValueError(f"Road feature {orig_idx} is missing a usable official link length.")
        geometry_feature_km = max(float(attrs.get("_geometry_length_km") or _km(pts)), 1e-9)
        ...
        for segment_idx, (start, end) in enumerate(zip(ordered, ordered[1:])):
            ...
            segment_geometry_km = _km(segment_pts)
            segment_km = segment_geometry_km / geometry_feature_km * source_feature_km
            if segment_km > 0:
                segment_attrs = {
                    **attrs,
                    "_segment_geometry_length_km": segment_geometry_km,
                    "_source_feature_length_km": source_feature_km,
                }
                noded_score_roads.append((orig_idx, segment_idx, segment_pts, segment_attrs, segment_km))
    ...
    for orig_idx, segment_idx, pts, attrs, km in noded_score_roads:
        ...
        data = dict(
            ...
            length_km=km,
            length_source=attrs.get("_model_length_source", "missing_official_link_length"),
            geometry_length_km=float(attrs.get("_segment_geometry_length_km", km)),
            source_feature_length_km=float(attrs.get("_source_feature_length_km", np.nan)),
            aadt=aadt,
    ...
    if cfg.helipads:
        ...
        for i, (lon, lat, name, notes) in enumerate(cfg.helipads, start=1):
            ...
            G.add_node(
                ...
                airport_type="Additional air-access point",
                helipad_notes=notes,
                abbrev=f"HP{i:02d}",
            )
            G.add_edge(
                ...
                node_id,
                type="airport_access",
                road_class="Additional air access",
                length_km=near_d,
                aadt=0,
                zone="Air",
                capacity=5,
            ...
            helipads_linked += 1
        print(f"  additional air-access points linked: {helipads_linked}")
...
def assign_socioeconomic_weights(
    ...
    if table.empty:
        raise RuntimeError("07_Population_GDP is empty; district weights cannot be constructed.")
    ...
    if missing:
        raise RuntimeError(f"07_Population_GDP lacks required columns: {', '.join(missing)}")
    ...
    if len(records) != 77:
        raise RuntimeError(f"Expected 77 auditable district rows in 07_Population_GDP; found {len(records)}.")
...
def _assignment_graph(cfg: MLPIConfig, G: nx.Graph, include_air: bool = True) -> nx.Graph:
    ...
    for u, v, d in G.edges(data=True):
        ...
        if edge_type not in {"road", "airport_access", "air_route"}:
            continue
        ...
        R.add_edge(
            ...
            t0=t0,
            length_km=length,
            capacity=max(float(d.get("aadt", cfg.aadt_default)), 300.0) if edge_type == "road" else 10000.0,
...
def _parse_odme_zone_label(label: Any) -> Dict[str, str]:
    text = str(label or "").strip()
    match = ODME_ZONE_LABEL_RE.match(text)
    if match:
        return {
            "zone_id": (match.group("zone_id") or "").strip(),
    ...
    raise ValueError(
        f"Could not parse ODME zone label '{text}'. Expected labels like "
        "'Achham | Mangalsen (Achham)'."
    )


def _read_odme_matrix(cfg: MLPIConfig) -> pd.DataFrame:
    ...
    if raw.index.duplicated().any():
        ...
        raise ValueError(f"ODME matrix has duplicate row labels: {dupes[:5]}")
    if pd.Index(raw.columns).duplicated().any():
        dupes = pd.Index(raw.columns)[pd.Index(raw.columns).duplicated()].tolist()
        raise ValueError(f"ODME matrix has duplicate column labels: {dupes[:5]}")
    if set(raw.index) != set(raw.columns):
        ...
        raise ValueError(
            "ODME matrix row/column labels do not match. "
    ...
    if len(bad):
        ...
        raise ValueError(f"ODME matrix has non-numeric cells at: {sample}")
    values = numeric.to_numpy(dtype=float)
    if not np.isfinite(values).all():
        raise ValueError("ODME matrix contains infinite values.")
    if (values < -1e-9).any():
        raise ValueError("ODME matrix contains negative demand values.")
...
def _district_anchor_lookup(cfg: MLPIConfig) -> Dict[str, Tuple[float, float]]:
    ...
    name_col = normalized.get("district_name") or normalized.get("district") or normalized.get("city") or normalized.get("node")
...
def _zone_anchor_point(
    ...
    if district_key in district_anchors:
        return district_anchors[district_key], "district anchor from 07_Population_GDP"
    return None, "no coordinate anchor available"


def _build_odme_zone_mapping(
    cfg: MLPIConfig,
    G: nx.Graph,
    R: nx.Graph,
    labels: Sequence[str],
    ...
    if not road_nodes:
        raise RuntimeError("No road nodes are available for ODME zone mapping.")
    ...
    for label in labels:
        parsed = _parse_odme_zone_label(label)
    ...
    if missing:
        raise RuntimeError(
            "Could not map every ODME zone to the MLPI road graph: "
            + "; ".join(missing[:12])
        )
...
def load_odme_pairs(cfg: MLPIConfig, G: nx.Graph, R: nx.Graph) -> List[Dict[str, Any]]:
    matrix = _read_odme_matrix(cfg)
    labels = list(matrix.index)
    parsed_by_label = {label: _parse_odme_zone_label(label) for label in labels}
    zone_mapping = _build_odme_zone_mapping(cfg, G, R, labels)

    ods: List[Dict[str, Any]] = []
    raw_positive_demand = 0.0
    total_positive_demand = 0.0
    below_threshold_pairs = 0
    below_threshold_demand = 0.0
    same_node_pairs = 0
    same_node_demand = 0.0
    min_demand = max(0.0, float(cfg.min_od_pair_demand))
    for i, origin_label in enumerate(labels):
        ...
        for j, destination_label in enumerate(labels):
            ...
            if demand <= 0.0:
                continue
            raw_positive_demand += demand
            if demand < min_demand:
                below_threshold_pairs += 1
                below_threshold_demand += demand
                continue
            ...
            ods.append(
                dict(
                    ...
                    od_model="external ODME matrix",
                    od_source=str(cfg.od_matrix_path),
                    demand_units=cfg.road_aadt_units,
    ...
    cfg.od_matrix_raw_positive_pcu_per_day = raw_positive_demand
    cfg.od_matrix_total_pcu_per_day = total_positive_demand
    cfg.od_pairs_loaded = len(ods)
    cfg.od_pairs_below_threshold_skipped = below_threshold_pairs
    cfg.od_pcu_below_threshold_skipped = below_threshold_demand
    cfg.od_pairs_same_node_skipped = same_node_pairs
    cfg.od_same_node_pcu_skipped = same_node_demand
    if not ods:
        raise RuntimeError(
            f"ODME matrix {cfg.od_matrix_path} has no positive inter-node OD pairs after zone mapping."
        )
    print(
        f"  ODME matrix loaded: {len(labels)} zones | {len(ods)} positive inter-node OD pairs | "
        f"matrix demand {total_positive_demand:,.3f} {cfg.road_aadt_units}"
    )
    if below_threshold_pairs:
        print(
            f"  ODME pairs below {min_demand:g} {cfg.road_aadt_units} skipped: {below_threshold_pairs} "
            f"({below_threshold_demand:,.3f} {cfg.road_aadt_units})"
        )
    if same_node_pairs:
        print(
            f"  ODME same-node pairs skipped: {same_node_pairs} "
            f"({same_node_demand:,.3f} {cfg.road_aadt_units})"
        )
    return ods
...
def build_gravity_od(cfg: MLPIConfig, G: nx.Graph, R: nx.Graph) -> List[Dict[str, float]]:
    raise RuntimeError(
        "MLPI requires load_odme_pairs() with the calibrated ODME CSV."
    )
...
def compute_physical_fftdi(cfg: MLPIConfig, G: nx.Graph) -> Tuple[Dict[Tuple[int, int], Dict[str, float]], pd.DataFrame, List[Dict[str, float]]]:
    """FFTDI-style physical importance from ODME loading and link removal."""
    R = _assignment_graph(cfg, G)
    ods = load_odme_pairs(cfg, G, R)
...
def build_junction_links(
    ...
    for path in paths:
        ...
        geometry_length_km = sum(float(data.get("geometry_length_km", data.get("length_km", 0.0))) for data in edge_data)
        design_speeds = [
        ...
        links.append(
            dict(
                ...
                length_source=dominant("length_source", "Nepal.gpkg NH link_len proportional allocation"),
                direct_km=direct_km,
...
def compute_section_physical(
    ...
    odme_top_od_pair: Dict[str, str] = {}
    odme_top_od_origin: Dict[str, str] = {}
    odme_top_od_destination: Dict[str, str] = {}
    odme_top_od_origin_district: Dict[str, str] = {}
    odme_top_od_destination_district: Dict[str, str] = {}
    odme_top_od_pcu_per_day: Dict[str, float] = {}
    odme_top_od_base_time: Dict[str, float] = {}
    odme_top_od_removed_time: Dict[str, float] = {}
    odme_top_od_delta_time: Dict[str, float] = {}
    odme_top_od_pcu_hour_loss: Dict[str, float] = {}
    odme_top5_od_pairs: Dict[str, str] = {}
    ...
    cfg.od_baseline_connected_pcu_per_day = sum(float(od["demand"]) for od in base_od_records)
    cfg.od_baseline_disconnected_pairs = sum(1 for od in ods if not od.get("base_connected"))
    cfg.od_baseline_disconnected_pcu_per_day = sum(float(od["demand"]) for od in ods if not od.get("base_connected"))
    if cfg.od_baseline_disconnected_pairs:
        disconnected_share = cfg.od_baseline_disconnected_pcu_per_day / max(
            cfg.od_baseline_connected_pcu_per_day + cfg.od_baseline_disconnected_pcu_per_day,
            1e-9,
        )
        warnings.warn(
            f"ODME baseline has {cfg.od_baseline_disconnected_pairs} disconnected OD pairs "
    ...
    if not base_od_records:
        raise RuntimeError(
            "ODME matrix loaded, but no positive OD pair is connected on the MLPI road graph. "
            "Check the OD zone mapping, road layer filter, and repaired road topology."
        )
    ...
    for section in sections:
        ...
        if top_od_rows:
            top_od_rows.sort(
                key=lambda row: (
                    float(row.get("pcu_hour_loss", 0.0)),
                    float(row.get("demand", 0.0)),
                ),
                reverse=True,
            )
            top = top_od_rows[0]
            odme_top_od_origin[sid] = str(top.get("origin_zone_label", ""))
            odme_top_od_destination[sid] = str(top.get("destination_zone_label", ""))
            odme_top_od_origin_district[sid] = str(top.get("origin_district", ""))
            odme_top_od_destination_district[sid] = str(top.get("destination_district", ""))
            odme_top_od_pcu_per_day[sid] = float(top.get("demand", 0.0))
            odme_top_od_base_time[sid] = float(top.get("base_time", 0.0))
            odme_top_od_removed_time[sid] = float(top.get("removed_time", 0.0))
            odme_top_od_delta_time[sid] = float(top.get("delta_time", 0.0))
            odme_top_od_pcu_hour_loss[sid] = float(top.get("pcu_hour_loss", 0.0))
            odme_top_od_pair[sid] = (
                f"{top.get('origin_zone_label', '')} -> "
                f"{top.get('destination_zone_label', '')}"
            )
            odme_top5_od_pairs[sid] = " ; ".join(
                (
                    f"{row.get('origin_district', '')}->{row.get('destination_district', '')} "
                    f"q={float(row.get('demand', 0.0)):,.0f}, "
            ...
            odme_top_od_pair[sid] = ""
            odme_top_od_origin[sid] = ""
            odme_top_od_destination[sid] = ""
            odme_top_od_origin_district[sid] = ""
            odme_top_od_destination_district[sid] = ""
            odme_top_od_pcu_per_day[sid] = 0.0
            odme_top_od_base_time[sid] = 0.0
            odme_top_od_removed_time[sid] = 0.0
            odme_top_od_delta_time[sid] = 0.0
            odme_top_od_pcu_hour_loss[sid] = 0.0
            odme_top5_od_pairs[sid] = ""
    ...
    out = {
        s["section_id"]: dict(
            ...
            pcu_hour_loss=pcu_hour_loss.get(s["section_id"], 0.0),
            od_pcu_hour_loss=pcu_hour_loss.get(s["section_id"], 0.0),
            ...
            odme_top_od_pair=odme_top_od_pair.get(s["section_id"], ""),
            odme_top_od_origin=odme_top_od_origin.get(s["section_id"], ""),
            odme_top_od_destination=odme_top_od_destination.get(s["section_id"], ""),
            odme_top_od_origin_district=odme_top_od_origin_district.get(s["section_id"], ""),
            odme_top_od_destination_district=odme_top_od_destination_district.get(s["section_id"], ""),
            odme_top_od_pcu_per_day=odme_top_od_pcu_per_day.get(s["section_id"], 0.0),
            odme_top_od_base_time_h=odme_top_od_base_time.get(s["section_id"], 0.0),
            odme_top_od_removed_time_h=odme_top_od_removed_time.get(s["section_id"], 0.0),
            odme_top_od_delta_time_h=odme_top_od_delta_time.get(s["section_id"], 0.0),
            odme_top_od_pcu_hour_loss=odme_top_od_pcu_hour_loss.get(s["section_id"], 0.0),
            odme_top5_od_pairs=odme_top5_od_pairs.get(s["section_id"], ""),
...
def compute_section_economic(
    ...
    for section in sections:
        ...
        raw_e1[sid] = float(physical.get(sid, {}).get("pcu_hour_loss", 0.0)) * trade_exposure
        after_access = gateway_access(R2)
        raw_e2[sid] = sum(
...
def write_methodology_workbook(
    ...
    default_formula_rows = [
        ...
        ("Assignment", "Dijkstra free-flow shortest path", "min_path SUM(edge_travel_time_h)", "Each edge travel time is computed before routing; Dijkstra selects the route with the minimum total travel time. The demand surface is the externally generated ODME matrix."),
        ("ODME", "External calibrated matrix", "q_ij is read from output/OD/od_matrix.csv", f"The CSV is treated as the calibrated symmetric district/HQ ODME matrix in {cfg.road_aadt_units}; mirrored OD cells support total-flow link scoring, and pairs below {cfg.min_od_pair_demand:g} {cfg.road_aadt_units} are excluded from physical link-removal scoring."),
        ("ODME", "Zone-node mapping", "district/HQ OD zones -> representative NH/SH road nodes", "Mapping first uses the conserved district socioeconomic allocation; district/HQ anchor nearest-node assignment is audited separately."),
        ("ODME", "Baseline path filter", "Only OD pairs connected on the baseline MLPI road graph contribute to TSTT loss", "Disconnected baseline OD pairs are written to odme_pairs_used.csv and should be checked against road filtering/connectivity."),
        ("Physical", "FFTDI_raw", "(TSTT_removed(link)-TSTT_base)/TSTT_base", f"TSTT is OD-weighted free-flow {cfg.road_aadt_units.replace('/day', '')}-hours on the repaired Nepal.gpkg road topology."),
        ...
        ("Economic", "E1", "OD_time_loss * section_trade_exposure", "Travel-time loss weighted by the failed link's AADT and border-gateway accessibility, so urban circulation links do not dominate trade criticality."),
    ...
    params = [
        ...
        ("Road AADT units", cfg.road_aadt_units),
        ("Mean speed factor", cfg.mean_speed_factor),
        ("Formula index source", "Implemented formulas in mlpi.py"),
        ...
        ("ODME matrix", str(cfg.od_matrix_path)),
        ("ODME raw positive demand", cfg.od_matrix_raw_positive_pcu_per_day),
        ("ODME minimum pair demand", cfg.min_od_pair_demand),
        ("ODME demand below threshold skipped", cfg.od_pcu_below_threshold_skipped),
        ("ODME total positive demand used", cfg.od_matrix_total_pcu_per_day),
        ("ODME inter-node pairs loaded", cfg.od_pairs_loaded),
        ("ODME below-threshold pairs skipped", cfg.od_pairs_below_threshold_skipped),
        ("ODME baseline connected pairs", cfg.od_baseline_connected_pairs),
        ("ODME baseline disconnected pairs", cfg.od_baseline_disconnected_pairs),
    ...
    references = [
        ...
        ("ODME", "Van Zuylen & Willumsen", "The most likely trip matrix estimated from traffic counts", "Transportation Research B, 1980", ""),
        ("ODME", "Spiess", "A maximum likelihood model for estimating origin-destination matrices", "Transportation Research B, 1987", ""),
    ...
    joint_rows = [
        ...
        ("ODME input", f"OD demand is read from {cfg.od_matrix_path} in {cfg.road_aadt_units}; pairs below {cfg.min_od_pair_demand:g} {cfg.road_aadt_units} are excluded before physical path scoring."),
        ...
        ("ODME limitation", "The OD matrix is accepted as the calibrated demand input. Baseline-disconnected mapped pairs are excluded from link-removal TSTT and are audited for network-connectivity review."),
        ...
        ("OD demand", f"The ODME matrix is the symmetric daily demand input for link-removal scoring after applying the {cfg.min_od_pair_demand:g} {cfg.road_aadt_units} minimum pair threshold."),
    ]
    try:
        with pd.ExcelWriter(out, engine="openpyxl") as writer:
            normalized_cols = [
                ...
                "odme_top_od_pair", "odme_top_od_origin", "odme_top_od_destination",
                "odme_top_od_pcu_per_day", "odme_top_od_delta_time_h", "affected_od_pair_count",
                "affected_od_demand", "length_km", "geometry_length_km", "length_source", "mean_aadt", "aadt_source",
            ...
            component_cols = [
                "rank", "section_id", "section_name", "macro_region", "length_km", "geometry_length_km", "length_source", "mean_aadt",
                "physical", "P1_FFTDI", "P2_detour_weighted_exposure", "P2_AADT_exposure", "P2_UE_flow", "disconnected_demand_raw",
                "FFTDI_raw", "pcu_hour_loss", "od_pcu_hour_loss",
                "affected_od_base_vht", "affected_od_removed_vht", "affected_od_demand", "affected_od_pair_count", "baseline_link_od_flow",
                "odme_top_od_pair", "odme_top_od_origin", "odme_top_od_destination",
                "odme_top_od_origin_district", "odme_top_od_destination_district",
                "odme_top_od_pcu_per_day", "odme_top_od_base_time_h", "odme_top_od_removed_time_h",
                "odme_top_od_delta_time_h", "odme_top_od_pcu_hour_loss", "odme_top5_od_pairs",
...
def write_methodology_figures(cfg: MLPIConfig) -> None:
    methodology_figure(
        ...
        [
            f"Nepal.gpkg NH graph + {cfg.road_aadt_field} ({cfg.road_aadt_units}) + {cfg.mean_speed_factor:.2f} design speed",
            f"External symmetric ODME matrix ({cfg.road_aadt_units})",
    ...
    methodology_figure(
        ...
        [
            ...
            "Remove one road link",
            "E1 trade-exposed travel-time loss",
...
def run_mlpi(
    ...
    print(f"ODME matrix  : {cfg.od_matrix_path}")
    ...
    section_candidate_cols = [
        ...
        "origin_anchor_distance_km", "destination_anchor_distance_km",
                "length_km", "geometry_length_km", "length_source", "direct_km", "detour_ratio", "edge_count", "mean_aadt",
    ...
    od_path = cfg.out_dir / "odme_pairs_used.csv"
    od_table.to_csv(od_path, index=False)
    od_zone_mapping_path = cfg.out_dir / "odme_zone_mapping.csv"
    cfg.od_zone_mapping_audit.to_csv(od_zone_mapping_path, index=False)
    print(
        f"  physical OD-TSTT FFTDI complete | ODME pairs: {len(od_table)} | "
        f"baseline connected: {cfg.od_baseline_connected_pairs} | written: {od_path}"
    )
    print(f"  ODME zone mapping written: {od_zone_mapping_path}")
    ...
    print("\nOutputs:")
    output_names = [
        "mlpi_ranked_links.csv", "junction_links_used.csv", "population_node_allocation.csv", "odme_pairs_used.csv", "odme_zone_mapping.csv", "airport_assets_used.csv", "mlpi_methodology_formulas.xlsx",
    ]
    if cfg.gen_maps:
        output_names = [
